...
import physics_engine.models  # pyright: ignore[reportUnusedImport] # noqa: F401  (import-for-side-effects)
...
def render_sidebar() -> tuple[FlowConditions, str]:
    ...
    with st.sidebar:
        ...
        nu = st.number_input(
            ...
            min_value=float(config.NU_MIN),
            max_value=float(config.NU_MAX),
            value=float(config.DEFAULT_NU),
            format="%.2e",
        ...
        rho = st.number_input(
            "Density, ρ (kg/m³)",
            min_value=float(config.RHO_MIN),
            max_value=float(config.RHO_MAX),
            value=float(config.DEFAULT_RHO),
        ...
        U = st.number_input(
            ...
            min_value=float(config.U_MIN),
            max_value=float(config.U_MAX),
            value=float(config.DEFAULT_U),
        ...
        dpdx = st.number_input(
            ...
            min_value=-1.0,
            max_value=1.0,
            value=float(config.DEFAULT_DPDX),
            step=0.05,
        ...
        x_max = st.number_input(
            ...
            min_value=float(config.X_MAX_MIN),
            max_value=float(config.X_MAX_MAX),
            value=float(max(config.DEFAULT_X_MAX, 1.0)),
